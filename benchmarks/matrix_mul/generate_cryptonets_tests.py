...
b_raw = [64,15,39,64]
print(f"sum with vec b : {n_rows_out,n_cols_out,n_channels_out} \n")
for i_out in range(n_rows_out):
    for j_out in range(n_cols_out):
        for k_out in range(n_channels_out):
            output[i_out, j_out, k_out]=output[i_out, j_out, k_out]+b_raw[k_out]
...
################# scaled_mean_pool_2d #####################################
function_name = "scaled_mean_pool_2d"
input1 = output
...
row_offset = 0
for i_output in range(n_rows_out) : # n_rows_out
    col_offset = 0
    for j_output in range(n_cols_out): # n_cols_out
        for k_output in range(n_channels_output):
            for i_kernel in range(n_rows_kernel):
                for j_kernel in range(n_cols_kernel):
                    output[i_output, j_output, k_output] += input1[i_kernel + row_offset, j_kernel + col_offset, k_output]
...
pad_rows = max((n_rows_out - 1) * row_stride + n_rows_kernel - n_rows_in,0); # 3 
pad_cols = max((n_cols_out - 1) * col_stride + n_cols_kernel - n_cols_in,0); # 3
pad_top = int(pad_rows / 2)
pad_left =int(pad_cols / 2)
###########################
...
###############################################
# kernel w4 will be loaded from an existing file
...
with open("matrix_mul_io_example.txt", "w") as f:
    ...
    for i in range(1): # n_rows_out
        for j in range(1): # n_cols_out
            #for k in range(n_channels_out):
                f.write(f"{function_name}[{i}][{j}] {is_cipher} "+" ".join(f"{int(num)}" for num in output[i, j])+" "+" ".join(f"{0}" for _ in range(function_slot_count-n_channels_out))+"\n")
                #f.write(f"[{i}][{j}][{k}] {is_cipher} {int(output[i][j][k])} "+" ".join(f"{0}" for _ in range(function_slot_count-1))+"\n")
