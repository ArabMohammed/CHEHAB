...
void poly_reg(int slot_count)
{
  ...
  Input c4("c4",{i,j},Type::ciphertxt);
  Computation C("c_result",{i,j}, c1(i,j) - ( c0(i,j)*c0(i,j)*c4(i,j) + c0(i,j)*c3(i,j)+c2(i,j) ));
  C.evaluate(true);
}
...
int main(int argc, char **argv)
{
  ...
  int slot_count = 1024 ;
  const auto &func = Compiler::create_func(func_name,slot_count, 20, true, false);
  poly_reg(slot_count);
