...
impl CostFunction<VecLang> for VecCostFn<'_> {
        ...
        C: FnMut(Id) -> Self::Cost,
    {
        ...
        /*if matches!(enode, VecLang::Vec(_) ) { 
            let mut child_costs: Vec<usize> = vec![];
            enode.for_each(|id| child_costs.push(costs(id) as usize));
            let unique_costs: std::collections::HashSet<_> = child_costs.iter().collect();
            additional_cost = if unique_costs.len() > 1 { 10 } else { 0 };
        }*/
        // Check if child costs have different depths
