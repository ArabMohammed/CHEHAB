...
pub fn vector_assoc_min_rules(vector_width: usize) -> Vec<Rewrite<VecLang, ConstantFold>> {
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        rw!("assoc-fold-rot-sub-1"; 
        "(VecMinusRot (VecMinus ?x ?y) ?n))" => 
        "(VecMinus (VecMinusRot ?x ?n) (VecMinusRot ?y ?n))"
        if is_vec("?x","?y","?x","?y")
        ),
        rw!("assoc-fold-rot-sub-2"; 
        "(VecMinusRot (VecAdd ?x ?y) ?n))" => 
        "(VecAdd (VecMinusRot ?x ?n) (VecMinusRot ?y ?n))"
        if is_vec("?x","?y","?x","?y") 
        ),
        rw!("assoc-fold-rot-sub-3"; 
        "(VecMinus (VecMinusRot ?x ?n) (VecMinusRot ?y ?n)))" => 
        "(VecMinusRot (VecMinus ?x ?y) ?n)"
        if is_vec("?x","?y","?x","?y")
        ),
        rw!("assoc-fold-rot-sub-4"; 
        "(VecMinus (VecAddRot ?x ?n) (VecAddRot ?y ?n)))" => 
        "(VecAddRot (VecMinus ?x ?y) ?n)"
        if is_vec("?x","?y","?x","?y")
        ),
...
pub fn vector_assoc_add_rules(vector_width: usize) -> Vec<Rewrite<VecLang, ConstantFold>> {
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        ...
        **************************************************************************/
        rw!("assoc-fold-rot-add-1"; 
        "(VecAddRot (VecMinus ?x ?y) ?n))" => 
        "(VecMinus (VecAddRot ?x ?n) (VecAddRot ?y ?n))"
        if is_vec("?x","?y","?x","?y")
        ),
        rw!("assoc-fold-rot-add-2"; 
        "(VecAddRot (VecAdd ?x ?y) ?n))" => 
        "(VecAdd (VecAddRot ?x ?n) (VecAddRot ?y ?n))"
        if is_vec("?x","?y","?x","?y")
        ),
        rw!("assoc-fold-rot-add-3"; 
        "(VecAdd (VecAddRot ?x ?n) (VecAddRot ?y ?n)))" => 
        "(VecAddRot (VecAdd ?x ?y) ?n)"
        if is_vec("?x","?y","?x","?y")
        ),
        rw!("assoc-fold-rot-add-4"; 
        "(VecAdd (VecMinusRot ?x ?n) (VecMinusRot ?y ?n)))" => 
        "(VecMinusRot (VecAdd ?x ?y) ?n)"
        if is_vec("?x","?y","?x","?y")
        ),
...
pub fn vector_assoc_add_mul_rules(vector_width: usize) -> Vec<Rewrite<VecLang, ConstantFold>> {
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        rw!("assoc-fold-rot-mul-1";  
        "(VecMulRot (VecMul ?x ?y) ?n))" => 
        "(VecMul (VecMulRot ?x ?n) (VecMulRot ?y ?n))"
        if is_vec("?x","?y","?x","?y")
        ),
