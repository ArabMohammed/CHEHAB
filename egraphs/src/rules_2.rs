...
pub fn addition_rules(vector_width: usize, expression_depth: usize) -> Vec<Rewrite<VecLang, ConstantFold>>{
    ...
    max_vector_size = min(max_vector_size,MAX_VEC_SIZE);
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        rw!("add-0-0+0"; "0" => "(+ 0 0)"),
        rw!("add-a-a+0"; "?a" => 
        "(+ ?a 0)"
        ...
        rw!("neg-0-0+0"; "0" => "(- 0)"),
        /*rw!("part-fold-assoc-mul-add-1"; "(* (+ ?b ?c) ?a)" => 
        "(* a? (+ ?b ?c))"
        if is_leaf("?a","?a")
        ),
...
pub fn minus_rules(vector_width: usize, expression_depth: usize) -> Vec<Rewrite<VecLang, ConstantFold>>{
    ...
    let mut rules: Vec<Rewrite<VecLang, ConstantFold>> = vec![
        ...
        /*rw!("part-fold-assoc-add-sub-1"; "(- (+ ?b ?c) ?a)" => 
        "(- a? (+ ?b ?c))"
        if is_leaf("?a","?a")
        ),
        rw!("part-fold-assoc-add-sub-2"; "(- (* ?b ?c) ?a)" =>
        "(- a? (* ?b ?c))"
        if is_leaf("?a","?a")
        ),
        rw!("part-fold-assoc-sub-3"; "(- ?a (- ?b ?c))" => 
        "(+ (- ?a ?b) ?c)"
        ),
        rw!("part-fold-assoc-add-sub-4"; "(+ (- ?a ?b) ?c)" => "(- (+ ?a ?c) ?b)"),
        rw!("part-fold-assoc-add-sub-5"; "(- (- ?a ?b) ?c)" => "(- ?a (- ?c ?b))"),
...
pub fn multiplication_rules(vector_width: usize, expression_depth: usize) -> Vec<Rewrite<VecLang, ConstantFold>>{
    ...
    while initial_vector_size <= max_vector_size {
        ...
        if cond_check_any_elems_composed(initial_vector_size)
        ));
        initial_vector_size=initial_vector_size*2;
    }
    /********************************************/
